...
(function($) {
	...
	$.fn.treeview = function(settings) {
		var dfop = {
			...
			datatype : "json",
			/**
			 * @description {Config} url {String} Url for child nodes
			 *              retrieving.
			 */
			url : false,
			/**
			 * @description {Config} cbiconpath {String} Checkbox image path.
			 */
			cbiconpath : "./css/images/icons/",
			icons : [ "checkbox_0.gif", "checkbox_1.gif", "checkbox_2.gif" ],
			/**
			 * @description {Config} showcheck {Boolean} Whether to show check
			 *              box or not.
			 */
			showcheck : false,
			/**
			 ...
			 * @param {Object}
			 *            tree This tree object.
			 * @param {Object}
			 *            item Node item clicked on.
			 * @param {Number}
			 *            status 1 for checked, 0 for unchecked.
			 */
			oncheckboxclick : false,
			/**
			 * @description {Event} onnodeclick:function(tree, item) Fired when
			 *              a node is clicked on.
			 * @param {Object}
			 *            tree This tree object.
			 * @param {Object}
			 *            item Ndde item clicked on.
			 */
			onnodeclick : false,
			/**
			 * @description {Config} cascadecheck {Boolean} Whether node being
			 *              seleted leads to parent/sub node being selected.
			 */
			cascadecheck : true,
			/**
			 * @description {Config} data {Object} Tree theme. Three themes
			 *              provided. 'bbit-tree-lines' ,'bbit-tree-no-lines'
			 *              and 'bbit-tree-arrows'.
			 * @sample data:[{ id:"node1", //node id text:"node 1", //node text
			 *         for display. value:"1", //node value showcheck:false,
			 *         //whether to show checkbox checkstate:0, //Checkbox
			 *         checking state. 0 for unchecked, 1 for partial checked, 2
			 *         for checked. hasChildren:true, //If hasChildren and
			 *         complete set to true, and ChildNodes is empty, tree will
			 *         request server to get sub node. isexpand:false, //Expand
			 *         or collapse. complete:false, //See hasChildren.
			 *         ChildNodes:[] // child nodes }]
			 */
			data : null,
			/**
			 * @description {Config} clicktoggle {String} Whether to toggle node
			 *              when node clicked.
			 */
			clicktoggle : true,
			/**
			 * @description {Config} theme {String} Tree theme. Three themes
			 *              provided. 'bbit-tree-lines' ,'bbit-tree-no-lines'
			 *              and 'bbit-tree-arrows'.
			 */
		...
		function buildnode(nd, ht, deep, path, isend) {
			...
			if (deep == 1) {
				ht.push("<img class='bbit-tree-icon' src='" + dfop.cbiconpath
						+ "s.gif'/>");
			} else if (deep > 1) {
				ht.push("<img class='bbit-tree-icon' src='" + dfop.cbiconpath
						+ "s.gif'/>");
			...
			ht.push("<img class='bbit-tree-ec-icon ", cs.join(" "), "' src='"
					+ dfop.cbiconpath + "s.gif'/>");
			ht.push("<img class='bbit-tree-node-icon' src='" + dfop.cbiconpath
					+ "s.gif'/>");
			// checkbox
